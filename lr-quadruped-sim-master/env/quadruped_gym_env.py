...
class QuadrupedGymEnv(gym.Env):
  ...
  def __init__(
    ...
    Args:
      ...
      action_repeat: The number of simulation steps where the same actions are applied.o
  ...
  def setupObservationSpace(self):
    ...
    if self._observation_space_mode == "DEFAULT":
      ...
      observation_high = (np.concatenate((np.array([1.0, 0]),
                                          np.array([1.0]*4),
                                          np.array([np.inf]*3),
                                          np.array([np.inf]*3),
                                          ...
                                          np.array([np.inf]*12),
                                          np.array([1.0]*4))) +  OBSERVATION_EPS)
      observation_low = (np.concatenate((np.array([0.18, 0]),
                                          np.array([-1.0]*4),
                                          np.array([-np.inf]*3),
                                          np.array([-np.inf]*3),
  ...
  def _get_observation(self):
    ...
    if self._observation_space_mode == "DEFAULT":
      ...
      for i in range(4):
        ...
        foot_pos[3*i:3*i+3] = p
        foot_vel[3*i:3*i+3] = v
      self._observation = np.concatenate(([self.robot.GetBasePosition()[2], 0],
                                          self.robot.GetBaseOrientation(),
  ...
  def _reward_lr_course(self):
    ...
    forward_reward = current_base_position[0] - self._last_base_position[0]
    self._last_base_position = current_base_position
    energy_expenditure = self._time_step * abs(np.dot(self.robot.GetMotorTorques(), self.robot.GetMotorVelocities()))
    current_base_orientation = self.robot.GetBaseOrientation()
    yaw_cost = abs(current_base_orientation[2])
    current_base_velocity = self.robot.GetBaseLinearVelocity()
    velocity_error = abs(current_base_velocity[0] - 5)
    # clip reward to MAX_FWD_VELOCITY (avoid exploiting simulator dynamics)
    if MAX_FWD_VELOCITY < np.inf:
      # calculate what max distance can be over last time interval based on max allowed fwd velocity
      max_dist = MAX_FWD_VELOCITY * (self._time_step * self._action_repeat)
    ...
    return self._distance_weight * forward_reward - self._energy_weight * energy_expenditure -\
           self._yaw_weight * yaw_cost - self._velocity_weight * velocity_error + 0.01
  ...
  def _transform_action_to_motor_command(self, action):
    ...
    if self._motor_control_mode == "PD":
      ...
      action_joint = self._scale_helper(action, self._robot_config.LOWER_ANGLE_JOINT, self._robot_config.UPPER_ANGLE_JOINT)
      action_joint = np.clip(action_joint, self._robot_config.LOWER_ANGLE_JOINT, self._robot_config.UPPER_ANGLE_JOINT)
      action_cart = self.ScaleActionToCartesianPos(action)
      action = action_cart + action_joint
    else:
  ...
  def ScaleActionToCartesianPos(self,actions):
    ...
    scale_array = np.array([0.15, 0.05, 0.12]*4)
    ...
    for i in range(4):
      ...
      tau = np.matmul(np.transpose(J), (np.matmul(kpCartesian,(Pd-p)) - np.matmul(kdCartesian,v)))
